...
export async function searchRequest(searchQuery: SearchRequest, metadata: any, requestManager: RequestManager, stateManager: SourceStateManager, page_size: number): Promise<PagedResults> {
...
export async function retrieveStateData(stateManager: SourceStateManager) {
    ...
    const serverURL = (await stateManager.retrieve('serverURL') as string) ?? DEFAULT_KOMGA_SERVER_ADDRESS
